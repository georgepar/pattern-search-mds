...
theta_for_cut=0+(theta_step*(number_of_rows_in_sphere-number_of_rows_in_cut_sphere))
...
for ni1 in range(n):
	...
	for ni2 in range(n):
		value=0
		for di in range(3):
			value+=(points_in_cut_sphere[ni1][di]-points_in_cut_sphere[ni2][di])**2
		value=math.sqrt(value)
		value=round(value,7) #7 decimals
		eucl_temp.append(value)
		file_handler.write(str(value))
...
print("Cut sphere: Max distance under which neighbors are taken into account:",distance_for_closest_neighbors)
max_geod=max([geodesic_distances[i][j] for i in range(n) for j in range(n)])
