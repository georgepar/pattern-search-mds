...
number_of_rows_in_sphere = 35  # 1404 points
number_of_points_per_row = 39
number_of_rows_in_cut_sphere = 30
radius = 0.9
theta = 0
phi = 0
use_noise = False
...
phi_step = 2 * math.pi / number_of_points_per_row
theta_step = math.pi / number_of_rows_in_sphere
# the way the sphere is calculated is such that we need to cut from 0 to theta_for_cut to cut one pole
theta_for_cut = 0 + (
theta_step * (number_of_rows_in_sphere - number_of_rows_in_cut_sphere))

while theta <= math.pi:
    phi = 0
    while phi <= 2 * math.pi:
        x_coord = radius * math.sin(theta) * math.cos(phi)
        y_coord = radius * math.sin(theta) * math.sin(phi)
        z_coord = radius * math.cos(theta)
        ...
        if (theta >= theta_for_cut):
            points_in_cut_sphere.append([x_coord, y_coord, z_coord])
        phi += phi_step
    theta += theta_step

n = len(points)
# EUCLIDEAN DISTANCE CALCULATION FOR WHOLE SPHERE
eucl_distances = []
filename = "Euclidean_sphere.dat"
...
# GEODESIC DISTANCE CALCULATION FOR WHOLE SPHERE
geodesic_distances = []
neighbors_increasing = []
dijkstra_distances = []
visited_nodes = []
filename = "Geodesic_sphere.dat"
file_handler = open(filename, 'w')
...
print("Max distance under which neighbors are taken into account:",
...
####CALCULATION FOR CUT SPHERE

n = len(points_in_cut_sphere)
# EUCLIDEAN DISTANCE CALCULATION FOR CUT SPHERE
eucl_distances = []
filename = "Euclidean_cut_sphere.dat"
file_handler = open(filename, 'w')
for ni1 in range(n):
    eucl_temp = []
    for ni2 in range(n):
        ...
        for di in range(3):
            value += (
                     points_in_cut_sphere[ni1][di] - points_in_cut_sphere[ni2][
                         di]) ** 2
        value = math.sqrt(value)
        value = round(value, 7)  # 7 decimals
        eucl_temp.append(value)
...
# GEODESIC DISTANCE CALCULATION FOR CUT SPHERE
geodesic_distances = []
neighbors_increasing = []
dijkstra_distances = []
visited_nodes = []
filename = "Geodesic_cut_sphere.dat"
file_handler = open(filename, 'w')
...
print("Cut sphere: Max distance under which neighbors are taken into account:",
      distance_for_closest_neighbors)
max_geod = max([geodesic_distances[i][j] for i in range(n) for j in range(n)])
print("Cut sphere: Max geodesic distance between two points:", max_geod)

filename = "cut_sphere_coords.dat"
file_handler = open(filename, 'w')
for i in range(len(points_in_cut_sphere)):
    for j in range(3):
        file_handler.write(str(points_in_cut_sphere[i][j]))
        if (j < 2):
            file_handler.write(",")
    file_handler.write("\n")
file_handler.close()

print("Cut Sphere: Number of vectors:", len(points_in_cut_sphere))
...
if (show_plots):
    ...
    x_coords_hole = [k[0] for k in points_in_cut_sphere]
    y_coords_hole = [k[1] for k in points_in_cut_sphere]
    z_coords_hole = [k[2] for k in points_in_cut_sphere]
