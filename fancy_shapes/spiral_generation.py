...
while (angle<=angle_stop):
	for i in range(depth_of_spiral):
		x_coord=-0.9+(1.8*i)/(depth_of_spiral-1) #range is [-0.9,0,9]
		...
		if (angle<=angle_hole_stop and angle>=angle_hole_start and i<round(2*depth_of_spiral/3) and i>=round(depth_of_spiral/3)):
			pass
		else:
...
print("Max distance under which neighbors are taken into account (no holes):",distance_for_closest_neighbors)
...
for ni1 in range(n):
	...
	for ni2 in range(n):
		value=0
		for di in range(3):
			value+=(points_with_hole[ni1][di]-points_with_hole[ni2][di])**2
		value=math.sqrt(value)
		value=round(value,7) #7 decimals
		eucl_temp.append(value)
		file_handler.write(str(value))
...
print("Max distance under which neighbors are taken into account (hole):",distance_for_closest_neighbors)
max_geod=max([geodesic_distances[i][j] for i in range(n) for j in range(n)])
print("Max geodesic distance between two points:",max_geod)
