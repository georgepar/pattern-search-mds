...
ex = Experiment("3000 vectors - 3 -> 2 dimensions - Geodesic Ball Data - New version")
...
@ex.config
def cfg():
    coords_file = os.path.join(config.DATA_DIR, 'ball_coords_geopar.dat')
    distance_matrix_file = os.path.join(config.DATA_DIR,
                                        'Geodesic_ball_geopar.dat')
    target_dim = 2
